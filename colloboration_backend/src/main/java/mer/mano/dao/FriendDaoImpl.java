...
@Autowired
	...
	Session session=sessionFactory.getCurrentSession();
		SQLQuery sqlQuery=session.createSQLQuery("select * from user_mer where email in (select email from user_mer where email!=? minus (select fromId from frnd_mer where toId=? union select toId from frnd_mer where fromId=?))");
	...
	public List<User> listOfFriends(String email) {
		...
		Session session=sessionFactory.getCurrentSession();
		SQLQuery query=
		session.createSQLQuery("select * from user_mer where email in"
				+ "(select fromId from frnd_mer where toId=? and status='A' "
				+ " union "
				+ "select toId from frnd_mer where fromId=? and status='A')");
		query.setString(0, email);
